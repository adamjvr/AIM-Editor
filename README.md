...
For Apple targets, the preferred gate builds the **standalone macOS app + core tests** and the **standalone iPadOS app on a connected physical iPad concurrently**. The macOS branch launches the built desktop app as soon as its tests pass; the iPad branch verifies the signed product, installs it, and launches it on hardware:
...
The real JUCE 9.0.1 Linux application build/core tests are green through Pass 19, the real macOS standalone application build plus core CTest gate is green, and the physical iPadOS arm64 build/sign path is now green on hardware. Simulator verification remains retired: iPadOS is considered verified only when the signed app builds for `iphoneos`/arm64, installs on the connected physical iPad, and launches successfully.
